...
# 현재 환경에 맞는 .env 파일을 로드합니다.
env_path = '.env.prod' if os.getenv('ENV') == 'prod' else '.env.dev'
                ...
                if len(bought_list) < target_buy_count:
                    ...
                    if target_price < current_price:
                        ...
                        if buy_qty > 0:
                            ...
                            if sym in amex_symbol_list:
                                market = "AMEX"
                            result = buy(market=market1, code=sym, qty=buy_qty,
                                         price=get_current_price(market=market2, code=sym))
                            time.sleep(1)
                            if result:
                                soldout = False
                                bought_list.append(sym)
